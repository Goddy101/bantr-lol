...
const HOUSE_RATE = 0.04;      // 4.0% of Total Pot goes to Bantr.lol
const JACKPOT_RATE = 0.035;   // 3.5% of Total Pot goes to Weekly Jackpot
const ADMIN_RATE = 0.015;     // 1.5% of Total Pot goes to Group Admin
const AFFILIATE_RATE = 0.01;  // 1.0% of Total Pot goes to Direct Referrer
...
export const SettlementService = {
  ...
  async settleDuel(duelId: string, winnerId: string) {
    // 1. Fetch duel details and BOTH referrers
    const { data: duel, error: duelError } = await supabaseAdmin
      .from('duels')
      .select('*, creator:users!creator_id(referred_by_admin, referred_by_affiliate)')
      .eq('id', duelId)
      .single();
    ...
    // 2. Calculate the Base Math
    const totalPot = duel.stake_amount * 2;
    const totalCommission = totalPot * COMMISSION_RATE;
    const winnerPayout = totalPot - totalCommission;
    const jackpotCut = Math.floor(totalPot * JACKPOT_RATE);
    
    let houseCut = Math.floor(totalPot * HOUSE_RATE);
    let adminCut = Math.floor(totalPot * ADMIN_RATE);
    let affiliateCut = Math.floor(totalPot * AFFILIATE_RATE);

    // 3. Handle Missing Partners (House absorbs the cuts if nobody referred them)
    const adminId = duel.creator?.referred_by_admin || null;
    const affiliateId = duel.creator?.referred_by_affiliate || null;
    ...
    if (!affiliateId) {
      houseCut += affiliateCut;
      affiliateCut = 0;
    }

    // 4. Execute the Payout via RPC
    const { error: settlementError } = await supabaseAdmin.rpc('settle_match_payouts', {
      ...
      p_admin_id: adminId,
      p_admin_cut: adminCut,
      p_affiliate_id: affiliateId,
      p_affiliate_cut: affiliateCut
    });
    ...
    return { 
      success: true, 
      payouts: { winnerPayout, jackpotCut, houseCut, adminCut, affiliateCut } 
    };
  }
};
