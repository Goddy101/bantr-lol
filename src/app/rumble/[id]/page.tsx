...
export default async function RumblePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 1. Verify who is viewing (Optional for guests via share links)
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();

  // 2. Fetch the Rumble Pool using Admin Bypass
  ...
  // 3. Fetch all Participants (Public room data)
  ...
  // 4. Only query user-specific data if they are actually logged in
  if (user) {
  ...
  return (
    <RumbleClientUI 
      ...
      currentUserId={user?.id || null} // Safe for guests
      hasJoined={hasJoined}
