...
import DepositModal from "@/components/shared/DepositModal"; // <-- Added for the Zero-Balance Guard
...
export default function DuelClientUI({ duel, currentUserId, isLoggedIn }: DuelClientUIProps) {
  ...
  // Modal State for Zero-Balance Guard
  ...
  const handleShare = async () => {
    ...
    if (navigator.share && navigator.canShare(shareData)) {
      ...
      setIsCopied(true);
      toast.success("Link copied! Drop it in the group chat."); // <-- Sleek Toast
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success("Duel locked! Escrow secured.");
        router.push("/dashboard");
      } else {
        // --- ZERO BALANCE GUARD INTERCEPT ---
        // If the backend says they don't have enough money, pop the deposit modal!
        if (data.error.toLowerCase().includes("insufficient") || data.error.toLowerCase().includes("balance")) {
          toast.error("Insufficient funds! Fund your vault to match the stake.");
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white flex flex-col items-center justify-center p-4 pb-20 font-sans selection:bg-green-500/30 overflow-hidden relative">
      ...
      {/* The Deposit Modal for Zero-Balance Intercept */}
